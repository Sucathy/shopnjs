...
app.post("/upload", upload.array("product", 6), (req, res) => {
  let imageUrls = req.files.map((file) => {
    return `http://3.86.225.128:4000/images/${file.filename}`;
  });
...
app.post("/website", productUpload.array("website", 6), (req, res) => {
  let imageUrls = req.files.map(
    (file) => `http://3.86.225.128:4000/images/${file.filename}`
  );
...
app.post("/uploadeds", supload.single("image"), (req, res) => {
  ...
  const imageUrl = `http://3.86.225.128:4000/images/${req.file.filename}`;
  const modelUrl = `http://3.86.225.128:4000/models/${req.file.filename}.glb`;
...
//   const screenshotUrl = `http://3.86.225.128:4000/website_screenshots/${req.file.filename}`;
